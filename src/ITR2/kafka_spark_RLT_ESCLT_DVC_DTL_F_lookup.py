...
def filter_records(self, records, col_num, source, producer, topic):
    """
    input: records   a list of tuples
           col_num       number
           source        value
    ...
    filter_records = list()
    if source == None:
        kafka_msg = "The source is None return -2"
        producer.send_messages(topic, kafka_msg)
        return -2
    else:    
        for record in records:
            #record is tuple
            if record[col_num] == source:
                filter_records.append(tuple(record))
                #if all conditions are satisfied append record to output        
        
        return filter_records
        
        #lookup HR_ORG_HIER_D(0).HR_ORG_HIER_KY(1) 
        ...
        """
        multi_layer_filter(['HR_ORG_HIER_D','HR_PERS_D'], \
        [(1,3,source),(1,8,'A'),(0,8,(1,9),(0,6,(1,10)),(0,4,(1,11)),(0,2,(1,12))], \
        (0,1), \
        producer, topic)
        """ 
        
def multi_layer_filter(redis_table_strings, cndtn_tuples, dest_tuple, producer, topic):
    """
    input: redis_table_strings    list of strings
           cndtn_tuples           list of tuples
           dest_tuple             tuple
    return lookup_value           value
    """
    filter_tables = list()
    for index in range(len(redis_table_strings)):
        records = RedisOp(['get',redis_table_strings[index])
        filter_tables.append(list(eval(records[1].replace('[','').replace(']',''))))
        
    for item in list_cndtn_tuples:
        if !isinstance(item[2], tuple):
            # item[0] corresponds to table used
            # item[1] corresponds to num of column
            # item[2] corresponds to value used for filtering
            # number of column - 1 = item[1] - 1 = the index of data in tuples
            filter_tables[item[0]] = filter_records(filter_tables[item[0]],item[1]-1,item[2])
        else:
            for record in filter_tables[item[2][0]]:
                filter_tables[item[0]] = filter_records(filter_tables[item[0]],item[1]-1,record[item[2][1]])
    
    #if no record is found return 0
    if len(filter_tables[dest_tuple[0]]) == 0:
        return 0
    elif len(filter_tables[dest_tuple[0]]) == 1:
        return filter_tables[dest_tuple[0]][dest_tuple[1]]
    else:
        kafka_msg = "error! multiple records found! check conditions!"
        producer.send_messages(topic, kafka_msg)
...
class ITR2_Tables(object):
    ...
    def dstct(self, res, input_dict, output_list):
    ...
    def STR_transform(self, item, producer, topic_test):
        if item != None:
            return str(item)
        else:
            kafka_msg = "The value is None return nothing"
...
class HP_ESCLT_M1(ITR2_Tables):
    ...
    def LKUP_map(self, dest, redis_table, input_key_tuple_list, output_key, producer, topic_test):
        ...
        dest.__dict__[dest.get_key(output_key)] = super(HP_ESCLT_M1, self).dstct(r[1],input_dict,[0])
        input_dict.clear()
        return
    ...
    def HP_ESCLT_M1_process(self, dest, producer, topic_test):
        ...
        kafka_msg = "Doing HP_EMRG_CHG_RQR_FG -> TF_transform"
        producer.send_messages(topic_test, kafka_msg)         
        self.TF_transform('HP_EMRG_CHG_RQR_FG', producer, topic_test)
        
        kafka_msg = "Doing HP_RECUR_ISS_FG -> TF_transform"
        producer.send_messages(topic_test, kafka_msg)         
        self.TF_transform('HP_RECUR_ISS_FG', producer, topic_test)
        
        kafka_msg = "Doing HP_RBT_RQR_FG -> TF_transform"
        producer.send_messages(topic_test, kafka_msg)         
        self.TF_transform('HP_RBT_RQR_FG', producer, topic_test)
        
        kafka_msg = "Doing HP_VNDR_ENGMT_FG -> TF_transform"
        producer.send_messages(topic_test, kafka_msg)         
        self.TF_transform('HP_VNDR_ENGMT_FG', producer, topic_test)
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.NEXT_MTNG_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.NEXT_MTNG_DT <- ITR21.HPESCLTM1.HP_NXT_MTG_DT"
        producer.send_messages(topic_test, kafka_msg)             
        dest.__dict__[dest.get_key('NEXT_MTNG_DT_DAY_KY')] = self.get_value('HP_NXT_MTG_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.INCID_DSCVR_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.INCID_DSCVR_DT <- ITR21.HPESCLTM1.HP_INCID_DSCVR_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('INCID_DSCVR_DT_DAY_KY')] = self.get_value('HP_INCID_DSCVR_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ICC_ENGMT_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.ICC_ENGMT_DT <- ITR21.HPESCLTM1.HP_ICC_ENGMT_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('ICC_ENGMT_DT_DAY_KY')] = self.get_value('HP_ICC_ENGMT_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_REQ_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.ESCLT_REQ_DT <- ITR21.HPESCLTM1.HP_ESCLT_REQ_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('ESCLT_REQ_DT_DAY_KY')] = self.get_value('HP_ESCLT_REQ_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_OPEN_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.ESCLT_OPEN_DT <- ITR21.HPESCLTM1.HP_ESCLT_OPN_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('ESCLT_OPEN_DT_DAY_KY')] = self.get_value('HP_ESCLT_OPN_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.BUS_CNTNTY_RESTOR_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.BUS_CNTNTY_RESTOR_DT <- \
        ITR21.HPESCLTM1.HP_BUS_CNTNTY_RSTR_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('BUS_CNTNTY_RESTOR_DT_DAY_KY')] = self.get_value('HP_BUS_CNTNTY_RSTR_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_CLOSE_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.ESCLT_CLOSE_DT <- ITR21.HPESCLTM1.HP_ESCLT_CLOSE_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('ESCLT_CLOSE_DT_DAY_KY')] = self.get_value('HP_ESCLT_CLOSE_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_CNCL_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.ESCLT_CNCL_DT <- ITR21.HPESCLTM1.HP_ESCLT_CNCL_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('ESCLT_CNCL_DT_DAY_KY')] = self.get_value('HP_ESCLT_CNCL_DT')
        
        producer.send_messages(topic_test, "lookup started for HP_ESCLT_M1")
        
        kafka_msg = "lookup ASGN_GRP_KY from ITR23.ASGN_GRP_D where ITR23.ASGN_GRP_D.ASGN_GRP_NM = \
        ...
        kafka_msg = "lookup EMP_KY from ITR23.HR_PERS_D where ITR23.HR_PERS_D.EMAIL_ADDR_NM = ITR22.APP_ESCLT.ESCLT_MGR_EMAIL_NM"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HR_PERS_D', [(2,self.get_value('HP_ESCLT_MGR_EMAIL_NM'))], 'ESCLT_MGR_EMP_KY', producer, topic_test)
        
        kafka_msg = "lookup EMP_KY from ITR23.HR_PERS_D where ITR23.HR_PERS_D.EMAIL_ADDR_NM = ITR22.APP_ESCLT.ESCLT_CRT_EMAIL_NM"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HR_PERS_D', [(2,self.get_value('HP_ESCLT_CRT_EMAIL_NM'))], 'ESCLT_CRT_EMP_KY', producer, topic_test)
        
        kafka_msg = "lookup EMP_KY from ITR23.HR_PERS_D where ITR23.HR_PERS_D.EMAIL_ADDR_NM = ITR22.APP_ESCLT.REQR_EMAIL_NM"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HR_PERS_D', [(2,self.get_value('HP_REQR_EMAIL_NM'))], 'ESCLT_REQR_EMP_KY', producer, topic_test)
        
        kafka_msg = "MSTR_L_ESCLT_STAT_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where \
        MSTR_L_COL_ID='ML_ESCLT_STAT_KY' and VALU_ID = ITR22.APP_ESCLT.MSTR_L_ESCLT_STAT_KY = ITR21.HPESCLTM1.ML_ESCLT_STAT_KY"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HPSC_MSTR_L_CD_D', [(6,'ML_ESCLT_STAT_KY'),(7,self.get_value('ML_ESCLT_STAT_KY'))], \
        'MSTR_L_ESCLT_STAT_KY', producer, topic_test)
        
        kafka_msg = "MSTR_L_ESCLT_TYPE_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where MSTR_L_COL_ID='ML_ESCLT_TYPE_KY' and VALU_ID = \
        ITR22.APP_ESCLT.MSTR_L_ESCLT_TYPE_KY = ITR21.HPESCLTM1.ML_ESCLT_TYPE_KY"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HPSC_MSTR_L_CD_D', [(6,'ML_ESCLT_TYPE_KY'),(7,self.get_value('ML_ESCLT_TYPE_KY'))], \
        'MSTR_L_ESCLT_TYPE_KY', producer, topic_test)
        
        kafka_msg = "MSTR_L_ESCLT_TMLNSS_TYPE_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where \
        MSTR_L_COL_ID='ML_TMLNSS_TYPE_KY' and VALU_ID = ITR22.APP_ESCLT.MSTR_L_TMLNSS_TYPE_KY = ITR21.HPESCLTM1.ML_TMLNSS_TYPE_KY"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HPSC_MSTR_L_CD_D', [(6,'ML_TMLNSS_TYPE_KY'),(7,self.get_value('ML_TMLNSS_TYPE_KY'))], \
        'MSTR_L_ESCLT_TMLNSS_TYPE_KY', producer, topic_test)
        
        kafka_msg = "MSTR_L_DEV_ENGMT_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where MSTR_L_COL_ID='ML_DEV_ENGMT_KY' and VALU_ID = \
        ITR22.APP_ESCLT.MSTR_L_DEV_ENGMT_KY = ITR21.HPESCLTM1.ML_ESCLT_DEV_ENGMT_KY"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HPSC_MSTR_L_CD_D', [(6,'ML_DEV_ENGMT_KY'),(7,self.get_value('ML_ESCLT_DEV_ENGMT_KY'))], \
        'MSTR_L_DEV_ENGMT_KY', producer, topic_test)
        
        kafka_msg = "MSTR_L_DEFN_WARR_PER_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where \
        MSTR_L_COL_ID='ML_DEFN_WARR_PER_KY' and VALU_ID = ITR22.APP_ESCLT.MSTR_L_DEFN_WARR_PER_KY = ITR21.HPESCLTM1.ML_DEFN_WARR_PER_KY"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'HPSC_MSTR_L_CD_D', [(6,'ML_DEFN_WARR_PER_KY'),(7,self.get_value('ML_DEFN_WARR_PER_KY'))], \
...
class HP_ESCLT_DVC_M1(ITR2_Tables):
    ...
    def HP_ESCLT_DVC_M1_process(self, dest, producer, topic_test):
    
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_DVC_ID <- ITR22.APP_ESCLT_DVC.ESCLT_DVC_ID <- ITR21.HPESCLTDVCM1.HP_ESCLT_DVC_ID"
        producer.send_messages(topic_test, kafka_msg)             
        dest.__dict__[dest.get_key('ESCLT_DVC_ID')] = self.get_value('HP_ESCLT_DVC_ID')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.ESCLT_ID <- ITR22.APP_ESCLT_DVC.ESCLT_ID <- ITR21.HPESCLTDVCM1.HP_ESCLT_ID"
        producer.send_messages(topic_test, kafka_msg)    
        dest.__dict__[dest.get_key('ESCLT_ID')] = self.get_value('HP_ESCLT_ID')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.APP_PRTFL_ID <- ITR22.APP_ESCLT_DVC.APP_PRTFL_ID <- ITR21.HPESCLTDVCM1.HP_APP_PRTFL_ID"
        producer.send_messages(topic_test, kafka_msg)    
        dest.__dict__[dest.get_key('APP_PRTFL_ID')] = self.get_value('HP_APP_PRTFL_ID')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.LAST_AVL_EVNT_STRT_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.LAST_AVL_EVNT_STRT_DT \
        <- ITR21.HPESCLTDVCM1.HP_LAST_AVL_STRT_DT"
        producer.send_messages(topic_test, kafka_msg)  
        dest.__dict__[dest.get_key('LAST_AVL_EVNT_STRT_DT_DAY_KY')] = self.get_value('HP_LAST_AVL_STRT_DT')
        
        kafka_msg = "ITR23.ESCLT_DVC_DTL_F.LAST_AVL_EVNT_END_DT_DAY_KY <- ITR22.APP_ESCLT_DVC.LAST_AVL_EVNT_END_DT <- \
        ITR21.HPESCLTDVCM1.HP_LAST_AVL_END_DT"
        producer.send_messages(topic_test, kafka_msg)
        dest.__dict__[dest.get_key('LAST_AVL_EVNT_END_DT_DAY_KY')] = self.get_value('HP_LAST_AVL_END_DT')
        
        producer.send_messages(topic_test, "lookup started for HP_ESCLT_DVC_M1")
        
        kafka_msg = "lookup APP_KY from ITR23.APP_D where ITR23.APP_D.APP_CI_LGCL_NM = \
        ITR22.APP_ESCLT_DVC.ESCLT_CI_LGCL_NM = ITR21.HP_ESCLT_DVC_M1.LOGICAL_NAME"
        producer.send_messages(topic_test, kafka_msg)
        self.LKUP_map(dest, 'APP_D', [(1,self.get_value('LOGICAL_NAME'))], 'APP_KY', producer, topic_test)
        
        kafka_msg = "lookup CI_D_KY from ITR23.CI_D where ITR23.CI_D.CI_LGCL_NM = \
        ITR22.APP_ESCLT_DVC.ESCLT_CI_LGCL_NM = ITR21.HP_ESCLT_DVC_M1.LOGICAL_NAME"
        producer.send_messages(topic_test, kafka_msg)
        kafka_msg = str(self.get_value('LOGICAL_NAME'))
        producer.send_messages(topic_test, kafka_msg)
        
        #self.LKUP_map(dest, 'CI_D', [(1,self.get_value('LOGICAL_NAME'))], 'CI_D_KY', producer, topic_test)
        #kafka_msg = "The lookup value is " + dest.get_value('CI_D_KY')
        #producer.send_messages(topic_test, kafka_msg)
                
        kafka_msg = "IT_OWN_ASGN_GRP_ORG_HIER_KY <- lookup ASGN_GRP_ORG_HIER_KY from ITR23.ASGN_GRP_ORG_HIER_D \
        where ITR23.ASGN_GRP_ORG_HIER_D.ASGN_GRP_ORG_HIER_ID = ITR22.APP_ESCLT_DVC.IT_ASSET_OWN_ORG_HIER_ID = \
        ITR21.HPESCLTDVCM1.HP_IT_ASSET_OWN_ORG_HIER1_ID"
        producer.send_messages(topic_test, kafka_msg)
        ...
        kafka_msg = "SUPP_OWN_ASGN_GRP_ORG_HIER_KY <- lookup ASGN_GRP_ORG_HIER_KY from ITR23.ASGN_GRP_ORG_HIER_D \
        where ITR23.ASGN_GRP_ORG_HIER_D.ASGN_GRP_ORG_HIER_ID = ITR22.APP_ESCLT_DVC.SUPP_OWN_ORG_HIER_ID = \
        ITR21.HPESCLTDVCM1.HP_SUPP_OWN_ORG_HIER1_ID"
        producer.send_messages(topic_test, kafka_msg)
        ...
        kafka_msg = "MSTR_L_RGN_KY <- lookup HPSC_MSTR_L_KY from ITR23.HPSC_MSTR_L_CD_D where MSTR_L_COL_ID='ML_RGN_KY' and VALU_ID = \
        ITR22.APP_ESCLT_DVC.MSTR_L_RGN_KY = ITR21.HPESCLTDVCM1.ML_RGN_KY"
        producer.send_messages(topic_test, kafka_msg)
        ...
        MSTR_L_COL_ID='ML_FAIL_OVR_STAT_KY' and VALU_ID = ITR22.APP_ESCLT_DVC.MSTR_L_FAILOVR_STAT_KY = \
        ...
        MSTR_L_COL_ID='ML_ESCLT_TYPE_KY' and VALU_ID = ITR22.APP_ESCLT_DVC.MSTR_L_LAST_AVL_ESCLT_TYPE_KY = \
        ...
        MSTR_L_COL_ID='ML_TMLNSS_TYPE_KY' and VALU_ID = ITR22.APP_ESCLT_DVC.MSTR_L_LAST_AVL_TMLNSS_TYPE_KY = \
...
def process_data_HP_ESCLT_DVC_M1(iter):
    ...
    for record in iter:
        ...
        src_dvc_m1.__dict__[src_dvc_m1.get_key('HP_ESCLT_ID')] = 'EM1025232'
...
if __name__ == "__main__":
    ...
    topic_HP_ESCLT_DVC_M1 = 'HPESCLTDVCM1'
